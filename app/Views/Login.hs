...
pageContent = do
  H.div_ [ H.class_ "col-sm-8" ] $ do
    ...
    H.form_ [ H.class_ "form-inline", H.action_ "/login", H.method_ "post"  ] $ do
      H.div_ [ H.class_ "form-group" ] $ do
        H.label_ [ H.class_ "sr-only", H.for_ "inputPassword" ] "Password"
        H.input_ [ H.type_ "password"
                 , H.class_ "form-control"
                 , H.id_ "inputPassword"
                 , H.name_ "pwd"
                 , H.placeholder_ "Passwort" ]
      H.button_ [ H.type_ "submit", H.class_ "btn btn-default" ] "Ok"
